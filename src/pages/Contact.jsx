...
const contactInfo = [
  { icon: Mail, label: "Email", value: "rithickjacksonv@gmail.com", href: "mailto:rithickjacksonv@gmail.com" },
  { icon: MapPin, label: "Location", value: "Chennai, TN", href: null },
];
...
const ACCESS_KEY ="8719ce28-118e-4e3d-8b16-d53f2182a36c";
...
export default function Contact() {
  ...
  const input = (name) => ({
    width: "100%",
    background: "var(--bg)",
    border: `1px solid ${focused[name] ? "var(--g2)" : "var(--border2)"}`,
    borderRadius: "12px",
    padding: "11px 14px",
    fontSize: "13px",
    color: "var(--text)",
  ...
  return (
    ...
    >
      <div className="max-w-225 mx-auto">
        ...
        >
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.14em] mb-3"
            style={{ color: "var(--g2)" }}>
            <span className="w-5 h-px opacity-50" style={{ background: "var(--g2)" }} />
            Get In Touch
          </div>
          <h1 className="font-display font-extrabold tracking-tight mb-3"
            style={{ fontSize: "clamp(24px,5vw,52px)", color: "var(--text)" }}>
            Let's <span className="grad-text">Connect</span>
          </h1>
          <p className="text-[13px] md:text-[15px] leading-relaxed max-w-120"
        ...
        <div className="grid md:grid-cols-[1fr_1.6fr] gap-5 md:gap-8 items-start">
          ...
          <div className="flex flex-col gap-3 md:gap-5">
            ...
            {contactInfo.map(({ icon: Icon, label: lbl, value, href }, i) => (
              <motion.div key={lbl}
                ...
                <div className="w-9 h-9 md:w-11 md:h-11 rounded-[10px] flex items-center justify-center shrink-0"
                  style={{ background: "linear-gradient(135deg,rgba(37,99,235,0.08),rgba(124,58,237,0.13))", color: "var(--g2)" }}>
                  <Icon size={16} />
                </div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-[0.08em] mb-0.5"
                    style={{ color: "var(--softer)" }}>{lbl}</div>
                  {href
                    ? <a href={href} className="text-[13px] md:text-[14px] font-semibold no-underline"
                        style={{ color: "var(--text)" }}
                        onMouseEnter={(e) => (e.currentTarget.style.color = "var(--g2)")}
                        onMouseLeave={(e) => (e.currentTarget.style.color = "var(--text)")}>{value}</a>
                    : <span className="text-[13px] md:text-[14px] font-semibold" style={{ color: "var(--text)" }}>{value}</span>
                  }
                </div>
              </motion.div>
            ))}
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2 h-2 rounded-full shrink-0"
                  style={{ background: "#059669", boxShadow: "0 0 6px #059669", animation: "blink 1.8s ease-in-out infinite" }} />
                <span className="text-[11px] font-bold uppercase tracking-[0.08em]"
                  style={{ color: "#059669" }}>Available for work</span>
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
              ...
              <p className="text-[10px] md:text-[11px] font-bold uppercase tracking-widest mb-3"
                style={{ color: "var(--softer)" }}>Find me on</p>
              <div className="flex gap-2 md:gap-3 flex-wrap">
                {socials.map(({ icon: Icon, label: lbl, href, color }) => (
                  <motion.a key={lbl} href={href} target="_blank" rel="noopener noreferrer"
          ...
          <motion.div
            ...
            <h2 className="font-display font-bold text-[16px] md:text-[20px] mb-5"
              style={{ color: "var(--text)" }}>Send a Message</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                <div>
                  ...
                  <input type="text" name="name" required placeholder="Rithick"
                    style={input("name")} onFocus={() => handleFocus("name")} onBlur={() => handleBlur("name")} />
                </div>
                <div>
                  <label style={label}>Email</label>
                  <input type="email" name="email" required placeholder="you@email.com"
                    style={input("email")} onFocus={() => handleFocus("email")} onBlur={() => handleBlur("email")} />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="text" name="subject" required placeholder="What's this about?"
                  style={input("subject")} onFocus={() => handleFocus("subject")} onBlur={() => handleBlur("subject")} />
              </div>
              ...
              <div>
                ...
                <textarea name="message" required rows={5}
                  ...
                  style={{ ...input("message"), resize: "vertical", minHeight: "110px" }}
